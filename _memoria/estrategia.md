...
- **Progresso (jul/2026): esteira de áudios CONSTRUÍDA no Salesbot** (gatilho: entra em "Onboarding D0" do Funil Suporte → 8 toques texto+nota de voz → move pro Funil Anual). **Pulo do gato do áudio:** marcar **"Converter em áudio"** no anexo faz o Salesbot mandar como **nota de voz** (senão vai como documento). Áudios em `.ogg` (`dados/audios-degustacao/ogg-prontos/`). Passo a passo completo em `saidas/passo-a-passo-salesbot-esteira.md`. **Progresso:** (1) ✅ esteira testada ponta a ponta, rodando redonda; (2) ✅ handoff de entrada CONSTRUÍDO e testado ao vivo — script `scripts/guru-para-kommo.py` acha o lead pelo telefone da venda → move pro Funil Suporte/Onboarding D0 + tag CLIENTE DEGUSTAÇÃO → esteira dispara (áudio confirmado no WhatsApp). Fase manual documentada em `saidas/runbook-handoff-degustacao.md` (gatilho = painel/app da Guru; Gabriel roda o comando por venda). **Próximo:** (a) automatizar o handoff — ✅ endpoint CONSTRUÍDO e testado local (`api/handoff.py`, função serverless Vercel, Python puro): recebe webhook "venda aprovada" da Guru → valida api_token + filtra produto degustação → acha lead pelo telefone e move (ou CRIA se não existir) pro Suporte/Onboarding D0 + tag. Guia de deploy em `saidas/guia-deploy-webhook-guru.md`. PENDENTE (Gabriel): conectar repo na Vercel, setar 4 env vars (KOMMO_BASE_URL, KOMMO_LONG_LIVED_TOKEN, GURU_API_TOKEN, GURU_PRODUCT_IDS), plugar a URL na Guru e testar com venda real. Script manual `guru-para-kommo.py` fica como plano B. (b) bot do Funil Anual (depende dos áudios/vídeos novos do Henrique). IDs Kommo úteis: Funil Degustação 13257796, Funil Suporte 14050808, etapa Onboarding D0 108457804.
